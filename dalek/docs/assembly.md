...
Read the whole manual and open the CAD preview. Inventory `bom/electronics.csv`, `bom/hardware.csv`, and `bom/printed-parts.csv`. FACET-1 has ten STL designs. Print the shared pitch carrier twice and the optional motor clamp four times, giving fourteen printed pieces. The motor-tie option omits all four clamps and needs ten prints. The skirt is one 213 mm print with twelve full-height flat panels and a single straight taper; the shoulder and neck are one 186 mm print. Select one battery and its matching charger. Keep purchased screws, bearings, retainers and spacers in labelled bags.
...
Print `07_pitch_carrier.stl`, `10_head_motor_carriage.stl` and one `11_motor_clamp.stl` if using clamps first. Use them to check the real servo case, ears, supplied horn, TT case, sliding slots and screw fits. The servo shaft offset and ear thickness are measured fit gates. Then print `01_base.stl` and check the motors, fully seated wheels and selected retention method. Check each subsequent part's screw holes, bearing seats and board slots. Clean the first-layer edge before concluding that a dimension is wrong. Do not force a bearing or crush a board into a tight print.
...
Follow the orientation and quantity fields in `printed-parts.csv`. The body order is `01_base`, `02_skirt`, `04_shoulder`, and `06_head`; identifiers 03 and 05 are retired. Print each complete body section upright at 100% scale. Print `12_electronics_platform.stl` deck down with its four legs up, as exported. Its supports must be removed from every foot, key and fastening groove. Print four identical hook clamps only for the motor-screw option. The taller skirt and combined upper shell keep overall height at 563.8 mm.
...
The FACET-1 skirt slice uses 1009.47 g of PLA including supports and brim and predicts 23 hours 55 minutes 37 seconds; allow at least 1.12 kg with a 10 percent reserve. The unchanged combined upper shell uses 1235.49 g of PETG; allow at least 1.36 kg. Each job exceeds one 1 kg spool. Arrange compatible filament changes or sufficient spool capacity. The complete clamp-option package predicts 4180.82 g of filament, 2473.98 g of installed printed plastic and 124.28 hours of sequential printing. Filament purchase allowances are reserves, not predicted consumption. Match each printer job to its released mesh hash.
...
Follow the base and motor-retention steps in `mechanical.md`. Perform the physical proof-load test below before adding motors or electronics. Inspect the perimeter, floor, ribs and pocket walls. Retain the four standard M4 base-joint nuts in their underside pockets first. Each motor then uses one printed hook clamp with one M3x16 screw, washer and nut, or two longitudinal 4.8 mm ties. This is four motor-clamp screws total. Tighten only until the motor is secure. Do not put liquid thread-locker on printed plastic.
...
With the skirt off, place the disconnected battery on a 2 mm pad in the base cradle. Fill the spare space around smaller packs with firm nonconductive foam. Retain the pack with two separate padded ties, at least 500 mm long, 4.8 mm wide and rated at least 222 N. Keep terminals and wires out of the tie paths. Build the single board platform on the bench using the real PCB templates, its deck slots and 6 mm nonconductive spacers. Start with M2.5x20 PCB screws, or six M2x20 screws for the Pololu boards; trim and deburr to leave at least two full exposed threads and at most 3 mm projection below the deck. Board ties over clear insulated edges are an alternative.

Lower the populated platform over the battery onto its four rail seats. Use eight M3x12 screws with washers and nuts, or eight independent foot ties. The screw option uses the specified 2.5 mm L-key along Y from the side; the wheels can remain fitted. Keep terminal hardware below base-local Z130 and wire bundles inside the deck outline. The deck underside is Z140, its top is Z146, and PCB undersides are Z152. Before lowering the skirt, seat its four upper-joint standard M4 nuts from below and test each thread. Keep washers out of those pockets. After electrical bench tests, lower the whole skirt around the chassis and retain it with four M4x20 screws and screwhead washers into the captive base nuts. The platform does not pass through the skirt's smaller top opening.
...
Make the harness from the five circuit sheets and `electronics/wiring.csv`, using the listed wire gauges and connector ratings. Keep the single 7.5 A main fuse F1 within 100 mm of the intact Bioenno PP30 or the SLA positive terminal, including any adapter lead. Use matching insulated F2 contacts and a correctly polarized PP30 adapter for the specified SLA packs; an existing F1 pack needs F1 contacts. Keep the existing branch fuses and motor limits. Put the physical actuator switch in the feed to both movement converters. Keep the logic branch separately powered from MAIN. Terminate returns at the common ground point. Do not use a solderless breadboard or loose Dupont contacts for motor, servo or battery current.
...
Keep the arm horns and head drive connection detached. Command the positional channels to their documented centre pulse. Check that each servo stops and does not buzz against a hard limit. After the servo mounts are secured and their retaining tools are removed, fit the supplied horns at the neutral positions shown in the mechanical chapter. Retain each horn with its own centre screw. Do not print a replacement servo spline.

Keep the combined upper shell off the skirt on a padded stand. Fit the lower bearing, spacer, shims, retainers and spindle while the bottom is accessible, following the mechanical chapter. Leave the speaker and rear display out until both arm paths are complete. Retain each servo with two M2x10 socket-head ear screws or two 2.5 mm ties; neither option replaces the mandatory horn fasteners. Feed the detached arms through the rear of the lower opening, then turn their tips outward through the front sockets. Support their exposed stems. Insert the prepared carriers from below using the mechanical chapter's complete lower-entry path before seating them on the yaw horns. Use the exact multi-step paths in the mechanical chapter. Fit opaque stretch-fabric socket liners with loose folds. Use the current nominal 30 mm fixed outer radius and 24 mm moving inner radius, keeping the fixed edge inside the smaller box. No servo case, horn, tie or lead should be visible through a finished socket.
...
The two 608 bearings, spacer, shims, retainers and M8 spindle belong in the integral upper bearing tower. Fit the lower hardware through the open bottom before seating the upper shell. There is no separate neck joint. Fit the fifth TT motor and wheel to the sliding carriage on the bench, then lower that assembly through the top. Install all four slide nuts and underside washers through the open bottom. Keep the tyre retracted from the drum. Fit the speaker to its rear open frame, then fit the rear display and switches after both arm insertion paths are complete, while the bottom remains accessible. Complete these steps and the arm bench tests before lowering the upper shell onto the skirt. With the head off and carriage retracted, fit four M4x20 screws with screwhead washers into the standard nuts already captured below the skirt's upper flange. Use the driver routes in the mechanical chapter.
...
Confirm the speaker was fitted to its body mount after the arm assembly paths were complete and before the upper shell was seated. Keep both speaker terminals isolated from chassis ground. Wire the MAX98357A exactly as shown; its bridge speaker output has no grounded speaker lead. Confirm the gain resistor before applying power.
...
Test low-battery detection with a bench supply, not by repeatedly exhausting the battery. The retained limits stop motion at 11.2 V and permit explicit re-arm at 12.0 V. Firmware leaves controller/audio power on after a low-battery stop. Switch MAIN off and unplug the pack; an SLA battery has no Bioenno-style internal protection. Restore normal bench voltage and confirm recovery still requires explicit arming. Temporarily disconnect the servo-controller communication while safely supported, and confirm the documented fault response. Reconnect with power isolated.
...
Check the four complete body pieces: base, skirt, combined shoulder/neck and head. There are two bolted body joints, with eight M4x20 screws, eight captive standard M4 nuts and eight screwhead washers total. No washers belong inside the nut pockets. Check registers, rear screen, switches and service access. Route fixed cables clear of wheels, arms, the head track and screw tips. Leave labelled service loops. Confirm both battery ties and all platform feet hold their loads without movement.
...
Switch MAIN off, unplug the battery and remove it before charging. Use the Bioenno BPC-1502DC 14.6 V LiFePO4 charger only for BLF-1206A. Use the Power-Sonic PSC-121000ACX 12 V SLA charger for either specified SLA option; PSC-12500ACX is also specified for PS-1270. Label chargers by chemistry and check polarity. Never charge through the ESP32, its battery socket, an Adafruit single-cell charger or a converter output. Charge outside the body on a stable nonflammable surface with ventilation. AGM sealed lead-acid batteries must not operate or charge in an airtight enclosure. Recharge SLA after use and store it disconnected, following its manufacturer's instructions.
