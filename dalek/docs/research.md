...
FACET-1 changes the skirt to twelve full-height flat panels arranged around the circular300mm base. Each main face follows one linear taper from150mm vertex radius atZ0 to110mm atZ210. The old middle bend and exterior round band are removed. The four hemisphere rows sit normal to these faces, with raised circular bezels. The true panel wall thickness is1.8mm; circular joint collars and a faceted internal reinforcing rib retain the existing assembly datums. The skirt remains one213mm print and the robot remains563.8mm tall.

Four body sections stack vertically: base, faceted skirt, combined186mm shoulder/neck, and head. The other nine STL designs, all mounts and the90 x52 x103mm arm boxes retain their geometry. There are still ten STL files: fourteen prints with the four motor clamps, or ten when motor ties replace those clamps. The existing100-image inventory remains the appearance reference; this revision follows the user's specific request for actual flat panels while keeping the circular base.
...
The base uses PETG, a6mm floor, continuous perimeter walls and ribs, and integrated motor pockets. The selected structural slice uses six perimeter lines, six top and bottom layers and40% gyroid infill. Wheel wells extend through the inner top flange so the motor/wheel pairs can enter vertically; the outer3mm bumper band remains continuous. Four printed hook clamps use one screw per motor, or two independent ties per motor use dedicated staggered floor routes. The battery rests on the base. The single printed board platform stands above it and supports only the electronics. These are design specifications, not measured load ratings. The assembly guide specifies a physical proof-load test with the small gearmotors removed from the load path.
...
The design also accepts one Power-Sonic PS-1270 F2 12V7Ah or PS-12140 F2 12V14Ah lead-acid battery through the documented fused PP30 adapter. The combined supported bare envelope is100X x153Y x103Zmm, including terminals and manufacturer tolerance. Capacity alone does not specify case size. A157Y x104Xmm cradle provides room for fixed padding, and the platform leaves clearance for insulated terminals and wiring. Two long padded zip-tie loops retain the selected pack; foam blocks locate the smaller cases. Charge lead-acid batteries with their matched lead-acid charger, outside the robot. The existing LiFePO4 charger remains dedicated to its original pack. Exact data, sources and mass limits are in `battery-options-research.md` and the fifth circuit sheet.

The14Ah pack adds several kilograms compared with the Bioenno. Fit and electrical compatibility do not prove loaded traction, gearbox capacity or turning performance. Retain the existing5V motor supply and current limits. Weigh and test the finished robot on a level hard floor. After any low-battery stop, switch MAIN off and unplug the battery: firmware stops motion but does not disconnect the logic supply, and an SLA battery has no pack BMS to prevent continued discharge.
