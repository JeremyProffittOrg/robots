...
The MOUNT-1 design keeps its circular300mm base and563.8mm overall height, below three feet. Four complete body sections stack vertically: base,213mm one-piece skirt,186mm combined shoulder/neck, and head. Combining the upper shell removes one body joint while preserving head and arm positions. A common concealed pitch carrier, two decorative arms, sliding head-motor carriage, one hook-clamp design and one board platform complete the ten-STL set. Print the carrier twice and the optional motor clamp four times: fourteen prints with motor clamps, or ten when motor ties replace the clamps. The upper arm boxes shrink to90 x52 x103mm. The separate appearance chapter and existing100-image inventory document the retained style. The circular base is a requested customization; photographed bronze television props usually have faceted fenders.
