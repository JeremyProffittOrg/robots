...
link("5V_MOTOR", "U2.VOUT", "U5.VMotor", "U6.VMotor", "R12.1", gauge="18", color="orange")
link("5V_SERVO", "U3.VOUT", "J_SERVO.2", "U8.14", "U9.14", gauge="18", color="red", note="Five separate servo branches; never feed through PCA9685 V+ traces")
...
link("GND", "B1.PP30-", "U2.GND", "U3.GND", "U4.GND", "U5.GND", "U6.GND", "J_SERVO.1", gauge="18", color="black", note="Star ground; power returns do not pass through ESP32 or PCA board")
...
link("MOTOR_ENABLE", "U1.GPIO12", "U5.SLP", "U6.SLP")
link("SDA", "U1.GPIO21", "U10.SDA")
link("SCL", "U1.GPIO22", "U10.SCL")
link("SERVO_DISABLE", "U1.GPIO27", "U10.OE", "R6.2", "U8.1", "U8.4", "U8.10", "U8.13", "U9.1", note="R6 10k to3V3; REMOVE PCA board OE pull-down")
...
channels = [("LEFT_YAW","U8",2,3),("LEFT_PITCH","U8",5,6),
            ("RIGHT_YAW","U8",9,8),("RIGHT_PITCH","U8",12,11),("HEAD","U9",2,3)]
...
for pin in [4,10,13]:
    wire("5V_SERVO","U3.VOUT",f"U9.{pin}","26","red","Unused buffer OE high")
for ref,value,rail in [("C1","220uF 25V low-ESR","MOTOR_IN"),("C2","220uF 25V low-ESR","SERVO_IN"),
                       ...
                       ("C7","470uF 10V low-ESR","5V_LOGIC"),("C8","100nF ceramic","5V_SERVO"),("C9","100nF ceramic","5V_SERVO")]:
...
class Sheet:
    ...
    def save(self,name):
        self.text(35,self.h-24,"DALEK R1 | 2026-09-11 | Circuit + wiring.csv form one drawing set | Build validation is required","small")
        s="\n".join(self.items+["</svg>"])
        ET.fromstring(s)
...
def diagrams():
    ...
    s.box(35,330,475,255,"U2 / MOTOR 5V",["ACT_IN -> F2 3A -> MOTOR_IN -> VIN","Pololu D36V50F5 #4091","VOUT -> 5V_MOTOR -> U5/U6.VMotor","C1 220uF/25V: VIN(+) to GND(-)","C4/C5 470uF/10V: one at each driver","R15 100ohm/1W: output to GND","EN, PG and VRP: no connection","Ground return: 18AWG to star"])
    s.box(555,330,475,255,"U3 / SERVO 5V",["ACT_IN -> F3 3A -> SERVO_IN -> VIN","Pololu D36V50F5 #4091","VOUT -> 5V_SERVO -> distribution block","C2 220uF/25V: VIN(+) to GND(-)","C6 1000uF/10V at distribution block","R16 100ohm/1W: output to GND","EN, PG and VRP: no connection","5 separate 22AWG servo branches"])
    ...
    assignments=[("GPIO21","SDA -> U10.SDA"),("GPIO22","SCL -> U10.SCL"),("GPIO27","SERVO_DISABLE / OE"),("GPIO12","MOTOR_ENABLE / SLP"),("GPIO25","LEFT_IN1"),("GPIO26","LEFT_IN2"),("GPIO32","RIGHT_IN1"),("GPIO33","RIGHT_IN2"),("GPIO17","I2S_BCLK"),("GPIO13","I2S_LRC"),("GPIO15","I2S_DATA"),("GPIO39 (SVN)","BAT_ADC"),("GPIO36 (SVP)","ACT_OK"),("3V3 / GND","U10.VCC / common GND")]
    for i,(p,n) in enumerate(assignments):s.row(55,173+24*i,p,n,650)
    s.box(825,115,725,195,"Reserved board connections",["TFT: MOSI19, SCLK18, CS5, DC16, RST23, BL4","Buttons: GPIO0 and GPIO35; reserve both.","Onboard battery ADC34 and power14 are NOT pack sense.","GPIO12 has 10k pull-down: required low boot strap state.","GPIO15 connects only to amplifier DIN; no external bias.","GPIO2, 37, 38 and UART pins: no external connection."])
    s.box(825,340,725,195,"Hardware stopped on reset",["R1..R4: 10k from each motor input net to GND.","R5: 10k MOTOR_ENABLE to GND; both SLP pins join it.","R6: 10k SERVO_DISABLE to3V3 (see sheet03).","Firmware: SLP low, inputs low, OE high before devices init.","Arm only after voltage, link and physical switch checks.","Physical power OFF works even if firmware stops responding."])
    ...
    s=Sheet("03 / Five servo channels with true 5V signals","All chip numbers below are DIP-14 PHYSICAL pin numbers. U8/U9 are 74AHCT125, not 74HC125.",1600,1190)
    s.box(35,115,740,185,"U10 / Adafruit PCA9685 #815",["VCC <-3V3; GND <- common ground; SDA21; SCL22","I2C address0x40, address links open; frequency50Hz","V+ pin, V+ terminals and V+ header row: UNCONNECTED","Remove onboard OE pull-down; add R6 10k OE to3V3.","OE -> SERVO_DISABLE -> GPIO27. High disables outputs.","Only PWM pins0..4 feed the buffer inputs below."])
    s.box(825,115,725,185,"U8 and U9 power / enable",["Each IC: pin14 ->5V_SERVO; pin7 ->GND","C8/C9:100nF directly across14 and7, one per IC.","U8 pins1,4,10,13 ->SERVO_DISABLE; U9 pin1 same.","U9 unused: pins5,9,12 ->GND; pins4,10,13 ->5V_SERVO","U9 unused outputs6,8,11: no connection.","Buffer power turns OFF with servo power; no phantom supply."])
    y=355
    for c,(name,chip,a,out) in enumerate(channels):
        ...
        s.box(1005,y,545,102,f"SV{c} / {'FS5103R #154' if c==4 else 'MG92B #2307'}",[f"SIGNAL <- R{30+c}; R{40+c} 10k SIGNAL toGND","RED <-5V_SERVO; BLACK/BROWN <-GND"])
        y+=127
    s.box(35,1000,1515,140,"Assembly and calibration",["Use a soldered board and sockets; locate notch / pin 1 before inserting ICs. Separate 5V servo distribution from PCA V+.","Test 1500us with horns removed; fit horns at center. Begin at +/-2 degrees and 0.10Hz; check every linkage.","After clearance checks, normal default is +/-8 degrees at 0.40Hz. Calibrate head neutral in firmware.","Continuous head servo controls SPEED, not angle. All electronics stay in body; head has no electrical wires."])
    ...
    s.text(40,690,"D. Motor output circuit (repeat for all4 motors)","head")
    ...
    s.text(40,1010,"U5 bridgeB ->LEFT REAR; U6 bridgeA ->RIGHT FRONT; U6 bridgeB ->RIGHT REAR.","small")
    ...
    s.line(1130,755,1300,755);s.text(1310,760,"U5/U6.SLP","net")
    ...
    s.line(1130,895,1130,955);s.text(920,980,"GPIO27 / U10.OE / buffer /OE","net")
    s.text(870,1040,"Remove originalPCAOE pull-down.","small")
    s.text(870,1070,"Repeat10k pull-down on each drive input.","small")
...
def main():
    ...
    results={
        ...
        "motor_rail_design_A":4,"servo_rail_design_peak_A":5,
        "logic_rail_design_peak_A":.9,"peak_output_W":5*(4+5+.9),
        "estimated_peak_pack_A_at_11p2V_85pct":5*(4+5+.9)/(11.2*.85),
        "estimated_runtime_h_15W":72*.75/15,"estimated_runtime_h_25W":72*.75/25,
        "max_mass_kg":mass,"wheel_radius_m":radius,
