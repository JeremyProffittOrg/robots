...
$scratchName = 'dalek-video-audio-' + [guid]::NewGuid().ToString('N')
...
try {
    $synth.SelectVoice($story.narration_voice)
    $synth.Rate = 0
    $synth.Volume = 100
    ...
    foreach ($chapter in $story.chapters) {
        ...
        $start = [double]$chapter.start + 0.35
        if ($start + $duration -gt [double]$chapter.end - 0.2) {
            throw "Narration exceeds its chapter window: $($chapter.title), $duration seconds. Shorten the narration."
        }
    ...
    foreach ($cue in $story.audio_cues) {
        $mp3 = Join-Path $projectRoot ('firmware/data/audio/' + $cue.file)
        if (-not (Test-Path -LiteralPath $mp3 -PathType Leaf)) { throw "Missing original robot clip: $($cue.file)" }
        $clips.Add([pscustomobject]@{
            kind = 'robot'; start = [double]$cue.start; duration = (Get-AudioDuration $mp3)
            path = $mp3; text = $cue.text
        })
    }
    ...
    foreach ($clip in $ordered) {
        if ($clip.start -lt $previousEnd -or $clip.start + $clip.duration -gt $total) {
            throw "Audio cue overlaps another cue or exceeds the video: $($clip.text)"
        }
    ...
    for ($index = 0; $index -lt $ordered.Count; $index++) {
        ...
        $filter.Add("[$($inputIndex):a]loudnorm=I=-18:TP=-2:LRA=7,aformat=sample_rates=48000:channel_layouts=mono,adelay=$($delay):all=1[a$inputIndex]")
    ...
    [pscustomobject]@{
        output = $outputFile; voice = $story.narration_voice
        duration_seconds = $outputDuration; sample_rate = 48000; channels = 1
        full_decode = 'PASS'; overlapping_cues = 0
    ...
    if ($resolvedScratch.Equals($expectedScratch, [StringComparison]::OrdinalIgnoreCase) -and
        ...
        (Split-Path $resolvedScratch -Leaf) -match '^dalek-video-audio-[0-9a-f]{32}$') {
