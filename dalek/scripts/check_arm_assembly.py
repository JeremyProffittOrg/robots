...
case_points = np.array(np.meshgrid(np.linspace(4.05, 30.95, 6),
                                  np.linspace(-5.95, 5.95, 5),
                                  np.linspace(13.05, 48.95, 8), indexing="ij")).reshape(3, -1).T
results = []
for x, entry_x in ((-47, -29), (53, 29)):
    poses = [(entry_x, -44, z) for z in np.linspace(130, 46, 43)]
    ...
    for label, points in (("carrier", meshes["07_pitch_carrier"].vertices), ("pitch-case", case_points)):
        for obstacle_name, obstacle in (("held-arms", held_arms), ("shoulder", meshes["04_shoulder"])):
            hits = 0
            for pose in poses:
                hits += int(obstacle.contains(points + np.array(pose)).sum())
...
report = dict(design="ROUND-10", assembly_order="Center and install yaw servos; feed and support both arms; insert pitch-servo carriers; attach horns and liners; speaker last.",
              method="Deterministic mesh vertices and nominal pitch-servo interior grid at 87 insertion poses per side.",
