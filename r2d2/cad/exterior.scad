// Revision C exterior relief, mm. Reference: docs/appearance-research.md.
$fn=96;
...
module octagon_port(theta,z){
 ...
 color(DARK)cylinder(d=29,h=2.8,$fn=8);color(SILVER)translate([0,0,2.5])ring(9,7.5,1.3);}
}
...
module body_details(){
 ...
 for(a=[113,247,292]){
  color(SILVER)outline(a,17,26,32,1.1);
  color(DARK)panel(a,20,18,25,.7);
  color(SILVER)panel(a,20,17,3.2,3,.3);
 }
