...
POST_RAIL=310;            // MISUMI HFS5-2020
POST_RAIL_END=62.5;       // rod-end centre to rail bottom
ACTUATOR_X=55;            // parallel actuator axis, guide-local X
ACTUATOR_Y=0;
ACTUATOR_FIXED_PIN=-37;
ACTUATOR_CLOSED=147;
ACTUATOR_ROD_EYE=0;       // moving eye and rod-end centre share guide-local Z
CENTER_YAW_LIMIT=8;
...
AIR_FOOT_PITCH=15;
function foot_pitch(s)=let(h=center_lift(s),d=ANKLE_Z-WHEEL_AXLE_Z,r=sqrt(d*d+WHEEL_ROW_Y*WHEEL_ROW_Y),phi=atan2(WHEEL_ROW_Y,d))
 h<=0.001?0:h>=r-d?AIR_FOOT_PITCH:min(AIR_FOOT_PITCH,max(0,phi-acos((d+h)/r)));
