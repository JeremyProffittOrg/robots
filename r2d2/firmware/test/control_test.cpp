...
int main(){using namespace r2;long v;
 PhEnDrive bridge;
 assert(bridge.tick(90,100).duty==230);
 assert(bridge.tick(0,110).duty==0);
 assert(bridge.tick(-70,150).duty==0);
 assert(bridge.tick(-70,209).duty==0);
 assert(bridge.tick(-70,210).duty==179&&bridge.output.direction==-1);
 assert(bridge.tick(100,220).duty==0);
 assert(bridge.tick(100,320).duty==255&&bridge.output.direction==1);
 ...
 Ramp ramp;for(int t=0;t<=100;t+=10)ramp.tick(100,t);assert(ramp.value==5);
 for(int t=110;t<=200;t+=10)assert(ramp.tick(-100,t)>=0);
 assert(ramp.value==0);assert(ramp.tick(-100,299)==0);assert(ramp.tick(-100,300)<0);
 for(int y=-8;y<=8;y++){assert(fabsf(steeringServoDegrees(y))<25);}
 assert(fabsf(postContactMm()-45.038373f)<.01f&&centerFootOffsetMm(0)>90);
