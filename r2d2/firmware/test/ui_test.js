...
const buttons=[{speed:'1'},{speed:'-1'},{head:'-1'},{head:'1'}].map(dataset=>({dataset,setPointerCapture(){}}));
...
const context=vm.createContext({console,URLSearchParams,AbortController,AbortSignal,
 ...
 fetch:async(url,options={})=>{const body=options.body?Object.fromEntries(options.body):{};requests.push({url,body});let value={};
  ...
  if(url.endsWith('/status'))value={armed,healthy:true,volts:12.8};
...
(async()=>{
 ...
 buttons[0].onpointerdown({preventDefault(){},pointerId:1});await settle();assert.equal(last().speed,'35');assert.deepEqual(Object.keys(last()).sort(),['head','lease','seq','speed','turn']);
