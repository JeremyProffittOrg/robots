"""Slice the four Revision B structural prints with installed H2D profiles.
...
def main():
 ...
 for part in ['body_lower','body_upper','arm_left','arm_right']:
  with tempfile.TemporaryDirectory(prefix='r2-structure-') as name:
   ...
   rows.append({'part':f'stl/{part}.stl','sha256':hashlib.sha256((ROOT/'stl'/f'{part}.stl').read_bytes()).hexdigest(),'return_code':d['return_code'],'error_string':d['error_string'],'warning':plate['warning_message'],'predicted_mass_with_support_g':plate['filaments'][0]['total_used_g'],'predicted_time_s':plate['total_predication'],'bbox':plate['objects'][0]['bbox']})
   (ROOT/'cad/h2d-structure-check.json').write_text(json.dumps({'profile':'H2D 0.4mm / 0.20mm / Generic PETG / Textured PEI','overrides':overrides,'rows':rows,'physical_print':False},indent=2))
   print('PASS: H2D PETG slice '+part+'; '+str(round(rows[-1]['predicted_mass_with_support_g'],1))+' g including support',flush=True)
